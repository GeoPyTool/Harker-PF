...
class Harker_Extended(QMainWindow):
    def __init__(self, parent=None):
        QMainWindow.__init__(self, parent)
        self.init_data()
    ...
    def plot_data(self):
        if self.df.empty:
            ...
            # 找到所有包含'wt%'的列名
            wt_cols = self.df.filter(like='wt%').columns
            ...
            # 生成对应个数的subplot
            ax_list = fig.subplots(rows, 3)

            # 现在，ax_list是一个包含n个subplot的列表
            ...
            # 遍历ax_list和target_y_list
            for ax, target_y in itertools.zip_longest(ax_list.flatten(), target_y_list):
    ...
    def export_result(self):   
        if self.df.empty:
            ...
            df = pd.DataFrame()
            self.result_show = AppForm(df= df,title = 'Harker Result')
    ...
    def save_figure(self):
        file_name, _ = QFileDialog.getSaveFileName(self, 
                                                   'Save Figure', 
                                                    working_directory+ 'Harker', 'PNG Files (*.png);;JPEG Files (*.jpg *.jpeg);;SVG Files (*.svg);;PDF Files (*.pdf)')
        if file_name:
            try:
